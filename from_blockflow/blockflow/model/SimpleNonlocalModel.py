...
class NonlocalModel(EthanGraphCNNModel):
	...
	def build_non_local_nn(self):
		...
		self.current_A, _ = self.__non_local_res_gcnn_block('spatial_nl2', 4, 8, 'softmax', embedding_dropout=False)
		#spatial_embedding_V = self.current_V
		#spatial_embedding_A = spatial_logits_A

		#self.current_A = (1 / (1 + self.current_A))
		...
		self.current_V = tf.concat([self.current_V, temp_V[:, :, :-4]])
