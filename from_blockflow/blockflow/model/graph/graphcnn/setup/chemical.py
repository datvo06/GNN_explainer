...
def load_protein_dataset(dataset_name):
    import scipy.io
    import numpy as np
    from graphkv.algorithm.architecture.graphcnn import InputPipeline
    ...
    dataset = InputPipeline(vertices = np.array(samples_V), adjacency=np.array(samples_A), labels=np.reshape(labels, [-1]))
