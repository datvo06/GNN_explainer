...
_safe_define('dataset_name', 'cora',
             'Name of the dataset for this experiment.')
_safe_define('train_batch_size', 128,
             "Number of samples to process in a training batch.")
_safe_define('test_batch_size', 0,
             "Number of samples to process in a test batch(0 = all).")
_safe_define('BN_DECAY', 0.9, "Normalization decay used for BatchNorm.")
_safe_define('INIT_FACTOR', 1.0,
             'Initialization factor used to scale ALL weight variables initializations.')
_safe_define('RESULTS_FILE',
             './results/{DATASET_NAME}/results.txt', 'Save results to file.')
...
_safe_define('save_checkpoints', True,
             'Should create checkpoints based on checkpoint rules (True/False).')
_safe_define('snapshot_iter', 1000, 'Snapshot period.')
_safe_define('iterations_per_test', 5,
             'Training iterations per test iteration.')


_safe_define('summary_save', True, 'Should save summaries (True/False).')
_safe_define('summary_period', 5,
             'Save summaries after x iterations (Default 5).')
...
def get_regex_flag(s):
    pattern = re.compile(
        '|'.join(['{' + k + '}' for k in FLAGS.__flags.keys()]))
