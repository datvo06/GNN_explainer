...
class CellNode(object):
    ...
    def get_text(self):
        if self.is_sub:
            ...
            return ' '.join([subline.get_text()
                             for subline in self.sub_lines])
    ...
    while len(list_sub_cell) > 0:
        ...
        for cell in cell_list:
            if len(list_sub_cell) == 0:
                break
            if cell.name.split('_')[0] == list_sub_cell[i].name.split('_')[0] and cell.name.split('_')[1] == list_sub_cell[i].name.split('_')[1]:
...
class Graph():
    ...
    def __init__(self, json_dict, doc_img):
        self.doc_img = doc_img
        self.org_items = get_cell_from_cell_dict(json_dict, doc_img)
        ...
        self.nodes = self.rows + self.cols + self.table_cells + self.text_lines
        '''
        ...
        for cell in cell_list_top_down:
            ...
            for r in rights:
                if r in cell.rights or r == cell:
                    continue
                if not get_intersect_range_horizontal_proj(cell.get_bbox(), r.get_bbox()) > min(cell.h, r.h) * 0.3:
                    continue
                self.edges.append(
    ...
    def _clean_left_right_edges(self):
        for cell in self.text_lines:
            ...
            removes = [c for c in left_cells if c.x + c.w >
    ...
    def _build_top_down_edges(self, cell_list_left_right):
        for cell in cell_list_left_right:
            cell_collide = [other_cell
                            ...
                            if other_cell.y > cell.y + cell.h * 0.6 and
                            check_intersect_vertical_proj(
                                cell.get_bbox(), other_cell.get_bbox()
                            ) and cell != other_cell]
            for other_cell in cell_collide:
        ...
        for cell in cell_list_left_right:
            ...
            for b in bottoms:
                if b in cell.bottoms or b == cell:
                    continue
                if not get_intersect_range_vertical_proj(cell.get_bbox(), b.get_bbox()) > min(cell.w, b.w) * 0.3:
                    continue
                self.edges.append(
    ...
    def _detect_column(self):
        ...
        for cell in self.table_cells:
            ...
            for o_cell in self.table_cells:
                ...
                if abs(o_cell.x - cell.x) <= p_margin and abs(o_cell.w - cell.w) <= w_margin:
    ...
    def _detect_row(self):
        ...
        for cell in self.table_cells:
            ...
            for o_cell in self.table_cells:
                ...
                if abs(o_cell.y - cell.y) <= p_margin and abs(o_cell.h - cell.h) <= h_margin:
                    aligns.append(o_cell)
            used_cells.extend(aligns)
            if len(aligns) > 1:
                new_row = Row(aligns)
