...
class FrozenGraphWeightVector(Block):
    def __init__(self, bow_features_block, coord_features_block, adj_mat_block, model_pb_path,
                 name=None, weights_name="Graph-CNN"):
        """
        :param bow_features_block: Bag of Words block.
        :param coord_features_block: Coordinate features block.
        :param adj_mat_block: Adjacency matrix block.
        :param model_pb_path: Frozen model weights for the Graph-CNN model.
        :param name:
        """
    ...
    def implement(self):
        ...
        for n in graph_def.node:
            if ("weights" in n.name and self.weights_name in n.name) and n.name.split("/")[-1] == "read":
                print(n.name, sep="\n")  # print(i, n.name, n.attr, sep="\n")
                weights_name.append(n.name + ":0")
        ...
        self.output_weights = tf.import_graph_def(graph.as_graph_def(),
                                                  input_map=input_map,
                                                  return_elements=weights_name+['import/{}:0'.format('output_features')])
        self.output_tensor = self.output_weights.pop()  # The tensor 'import/output_features:0'
        ...
        temp = {
            'features': (self.output_tensor, None),  # logits
            'weights': (self.output_weights, None)
        }
